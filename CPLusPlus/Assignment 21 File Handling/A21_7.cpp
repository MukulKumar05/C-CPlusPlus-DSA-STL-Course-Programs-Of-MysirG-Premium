...
/*
A21.7 In question 3, define a member function to edit employee data
of given empid.
...
class Employee
{
    ...
    public:
        ...
        void editEmployeeDataInFile(const char fileName[], int employeeID, const char employeeName[], float employeeSalary)
        {
            ...
            string line;
            int firstNumericSubstringAsNumber, editStatus = 0;
            while(getline(fin, line))
            {
                string firstNumericSubstring = extractFirstNumericSubstringFromString(line);
                firstNumericSubstringAsNumber = numberofNumericString(firstNumericSubstring);
...
{
    Employee emp4;
    emp4.setEmployee(25, "Anju Yadav", 12000);
    emp4.editEmployeeDataInFile("employeesRecords.txt", 1012, "Rakesh Yadav", 65000);
    return 0;
}